...
int vtrm_update(u8* data)
{
    ...
    ret = lv1_write_virtual_uart(10, vuart_lpar_addr, len, &nwritten);
    if (ret < 0) return ret;
...
int vuart_wait_for_rx_data(u64 port)
{
    ...
    {
        result = lv1_get_virtual_uart_param(port, 4, &val);
...
int vtrm_retrieve(u8* data)
{
    ...
    ret = vuart_wait_for_rx_data(10);
    if (ret < 0) goto leave;

    ret = lv1_read_virtual_uart(10, vuart_lpar_addr, 4096, &nread);
    if (ret < 0) goto leave;
