...
sys_process_param_t __sys_process_param SYS_PROCESS_PARAM_SECTION = {
		...
		SYS_PROCESS_PARAM_VERSION_310_0,
		0x00310001ULL,
		1200,
		0x100000,
...
*/
#define STR_APP_NAME "Rebug Toolbox DECR"
...
void dump_root_key()
{
    ...
    if(c_firmware==4.81f &&  dex_mode) strcpy(version, "475d");  else
    if(c_firmware==4.82f &&  dex_mode) strcpy(version, "475d");  else
    if(c_firmware==4.83f &&  dex_mode) strcpy(version, "475d");  else
    if(c_firmware==4.84f &&  dex_mode) strcpy(version, "475d");  else	return;	
...
void pokeq( uint64_t addr, uint64_t val)
{
	if(c_firmware!=3.55f && c_firmware!=3.15f && c_firmware!=4.21f && c_firmware!=4.30f && c_firmware!=4.31f && c_firmware!=4.40f && c_firmware!=4.41f && c_firmware!=4.46f && c_firmware!=4.50f && c_firmware!=4.53f &&
	   c_firmware!=4.55f && c_firmware!=4.60f && c_firmware!=4.65f && c_firmware!=4.66f && c_firmware!=4.70f && c_firmware!=4.75f && c_firmware!=4.76f && c_firmware!=4.78f && c_firmware!=4.81f && c_firmware!=4.82f && c_firmware!=4.83f && c_firmware!=4.84f) return;
...
static double get_system_version(void)
{
	...
	if(crc==0x41721eULL) base=3.15f; else // ofw   3.41
	if(crc==0x419d7eULL) base=3.15f; else // rebug 3.41.3 , rogero 3.55 3.7a
...
void add_home_column()
{
		...
		add_xmb_suboption(xmb[col].member[xmb[col].size-1].option, &xmb[col].member[xmb[col].size-1].option_size, 0, (char*)"Soft (LPAR Reboot Only)",				(char*)"1");
		add_xmb_suboption(xmb[col].member[xmb[col].size-1].option, &xmb[col].member[xmb[col].size-1].option_size, 0, (char*)"Soft (LV2 System call)",					(char*)"2");
...
void parse_settings()
{
	...
	for(int m=1;m<MAX_XMB_ICONS; m++)
	{
		for(int n=0;n<xmb[m].size;n++ )
		{
			...
			//else if(!strcmp(oini, "check_sha1"))        {check_sha1      =val; fix_sha1();}
...
/*
			else if(!strcmp(oini, "lv1_debug"))			lv1_debug		=val;
*/
...
//	Utilities
	...
	if(c_firmware==3.15f || c_firmware==3.55f || c_firmware==4.21f || c_firmware==4.30f || c_firmware==4.31f || c_firmware==4.40f || c_firmware==4.41f || c_firmware==4.46f  || c_firmware==4.50f || c_firmware==4.53f || c_firmware==4.55f || c_firmware==4.60f || c_firmware==4.65f || c_firmware==4.66f || c_firmware==4.70f || c_firmware==4.75f || c_firmware==4.76f || c_firmware==4.78f || c_firmware==4.81f || c_firmware==4.82f || c_firmware==4.83f || c_firmware==4.84f)
	{
	...
	*/
	...
	if( (!dex_mode && (c_firmware==3.15f || c_firmware==3.55f || c_firmware==4.46f || c_firmware==4.65f || c_firmware==4.66f)) || c_firmware==4.21f || c_firmware==4.70f || c_firmware==4.75f || c_firmware==4.76f || c_firmware==4.78f || c_firmware==4.81f || c_firmware==4.82f || c_firmware==4.83f || c_firmware==4.84f)
	{
	...
	add_xmb_member(xmb[col].member, &xmb[col].size, (char*)"Enable HVPROC", (char*)"Enable full hypervisor logs.",
			/*type*/6, /*status*/2, /*icon*/xmb_icon_tool, 128, 128);

	add_xmb_member(xmb[col].member, &xmb[col].size, (char*)"Fix Broken Blu-ray\xE2\x84\xA2 Movie Playback", (char*)"Verifies DRL/CRL hashes and fixes Blu-ray\xE2\x84\xA2 movie playback.",
			/*type*/6, /*status*/2, /*icon*/xmb_icon_tool, 128, 128);			
...
//	SELECTOR
	...
	if(rex_compatible)
	{
		...
		if(rebug_mode)
		{
			add_xmb_option(xmb[col].member, &xmb[col].size, (char*)"XMB Operation Mode", (char*)"Switch between DECR and DEX XMB.",	(char*)"xmb_mode");
			add_xmb_suboption(xmb[col].member[xmb[col].size-1].option, &xmb[col].member[xmb[col].size-1].option_size, 0, (char*)"DECR",		(char*)"0");
		...
		xmb[col].member[xmb[col].size-1].icon=xmb_icon_tool;
         */
		if(c_firmware==4.78f || c_firmware==4.81f || c_firmware==4.82f || c_firmware==4.83f || c_firmware==4.84f)
		{
		...
		if((c_firmware==4.21f || c_firmware==4.30f || c_firmware==4.31f || c_firmware==4.40f || c_firmware==4.41f || c_firmware==4.46f || c_firmware==4.50f || c_firmware==4.53f || c_firmware==4.55f ||
		    c_firmware==4.60f || c_firmware==4.65f || c_firmware==4.66f || c_firmware==4.70f || c_firmware==4.75f || c_firmware==4.76f || c_firmware==4.78f || c_firmware==4.81f || c_firmware==4.82f || c_firmware==4.83f || c_firmware==4.84f) && cobra_compatible)
		{
...
/*
        add_xmb_option(xmb[col].member, &xmb[col].size, (char*)"Fix Broken Blu-ray\xE2\x84\xA2 Movie Playback", (char*)"Verifies DRL/CRL hashes and fixes Blu-ray\xE2\x84\xA2 movie playback.",    (char*)"check_sha1");
        add_xmb_suboption(xmb[col].member[xmb[col].size-1].option, &xmb[col].member[xmb[col].size-1].option_size, 0, (char*)"NO" ,                        (char*)"0");
        add_xmb_suboption(xmb[col].member[xmb[col].size-1].option, &xmb[col].member[xmb[col].size-1].option_size, 0, (char*)"YES",                        (char*)"1");
		xmb[col].member[xmb[col].size-1].option_selected=check_sha1; 
		xmb[col].member[xmb[col].size-1].icon=xmb_icon_tool;	*/	
...
*/
	...
	xmb[col].member[xmb[col].size-1].icon=xmb_icon_tool;
/*
    add_xmb_option(xmb[col].member, &xmb[col].size, (char*)"FULL Hypervisor Logs", (char*)"Enable FULL Hyperviso logging.",	(char*)"lv1_debug");
	add_xmb_suboption(xmb[col].member[xmb[col].size-1].option, &xmb[col].member[xmb[col].size-1].option_size, 0, (char*)STR_XC2_DISABLE,			(char*)"0");
	add_xmb_suboption(xmb[col].member[xmb[col].size-1].option, &xmb[col].member[xmb[col].size-1].option_size, 0, (char*)STR_XC2_ENABLE,				(char*)"1");
	xmb[col].member[xmb[col].size-1].option_selected=lv1_debug;
	xmb[col].member[xmb[col].size-1].icon=xmb_icon_tool;
*/
	if(first<xmb[col].size) xmb[col].first=first; else xmb[col].first=0;
...
//	CEX/DEX Patches
   ...
   //if(rex_compatible)
	...
	add_xmb_member(xmb[col].member, &xmb[col].size, (char*)"CEX/DEX Functions Not Available", (char*)"NOT Available on DECR.",
			/*type*/6, /*status*/2, /*icon*/xmb_icon_tool, 128, 128);
}
...
/*
// DECI protocol packets sent to port 127.0.0.1:1000
// to mimic connect/restart/shutdown/activate_bdemu_slot

// Establish HT/MT channel com
u8 deci_con[0x34]=     {0x30, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x34,
                        0x4D, 0x54, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
                        0x00, 0x00, 0x00, 0x02, 0x50, 0x53, 0x33, 0x5F,
                        0x4C, 0x50, 0x41, 0x52, 0x00, 0x00, 0x00, 0x00,
                        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00, 0x00, 0x00, 0x00};

// Activate BDEMU slot (offset 0x1f)
u8 deci_set[0x21]=     {0x30, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x21,
                        0x48, 0x54, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00,
                        0x00, 0x00, 0x00, 0x08, 0x2F, 0x64, 0x65, 0x76,
                        0x5F, 0x62, 0x64, 0x65, 0x6D, 0x75, 0x2F, 0x31, // << Slot number (0x31=slot#1)
                        0x00};

// Reboot PS3
u8 deci_reboot[0x28]=  {0x30, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x28,
                        0x4D, 0x54, 0x00, 0xFF, 0x00, 0x00, 0x00, 0x20,
                        0x00, 0x21, 0x00, 0x18, 0x21, 0x0A, 0x00, 0x00,
                        0x50, 0x53, 0x33, 0x5F, 0x4C, 0x50, 0x41, 0x52,
                        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

// Shutdown PS3
u8 deci_shutdown[0x18]={0x30, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x18,
                        0x4D, 0x54, 0x00, 0xFF, 0x00, 0x00, 0x00, 0x20,
                        0x00, 0x21, 0x00, 0x08, 0x20, 0x04, 0x00, 0x00};
static void set_bdemu(u8 _slot)
{
    int s=connect_to_server( (char*)"127.0.0.1", 1000);
    if(s >= 0)
    {
        if(send(s, &deci_con, sizeof(deci_con), 0) == sizeof(deci_con))
        {
            sys_timer_usleep(100000);
            deci_set[0x1f]=0x30+_slot;
            send(s, &deci_set, sizeof(deci_set), 0);
            sys_timer_usleep(100000);
        }
...
static void reboot_dex(u8 _mode)
{
    if(!dex_mode || _mode>1) return;
    ...
    if(s >= 0)
    {
        if(send(s, &deci_con, sizeof(deci_con), 0) == sizeof(deci_con))
        {
            ...
            if(_mode)
                send(s, &deci_reboot, sizeof(deci_reboot), 0);
            else
                send(s, &deci_shutdown, sizeof(deci_shutdown), 0);
...
void exit_app()
{
	...
	if(auto_reboot)
	{
		...
		//reboot_dex(1);
		//system_call_4(378, (uint64_t)5,(uint64_t)2,(uint64_t)0,(uint64_t)(uint32_t)0);
		//system_call_2(8,(uint64_t)0x6cdd, 1);
		system_call_3(379, 0x200 , NULL, 0);
	}   //lv2_sm_shutdown(0x8201, NULL, 0);
	/*else 
	if((auto_reboot) && cobra_mode==0)
	{
		//system_call_3(379, 0x200 , NULL, 0);
		//reboot_dex(1);
	}   lv2_sm_shutdown(0x8201, NULL, 0);*/
...
//system_call_1(8,(uint64_t)0x1ee9);
...
void shutdown_system(u8 mode) // X0-off X1-reboot, X=0 no prompt, X=1 prompt
{
    if( !(mode&0x10) || (mode&0x10 && net_used_ignore()))
    {
        ...
        if(mode&1)
        {
            {system_call_2(8,(uint64_t)0x6cdd, reboot)}
...
void shutdown_system(u8 mode) // X0-off X1-reboot, X=0 no prompt, X=1 prompt
{
	...
	{
		unload_modules();
		if(!(mode&1)&&!(am_i_in_cobra()))
		{
			system_call_4(379,0x1100,0,0,0);
		}
		else if(!(mode&1)&&(am_i_in_cobra()))
		{
			system_call_2(8,(uint64_t)0x6cdd, 0);
		}		
		else if((mode&1)&&!(am_i_in_cobra()))
		{
			lv2_sm_shutdown(0x8201, NULL, 0);
...
/*
cobra_mode==1
system_call_2(8,(uint64_t)0x6cdd, 0); //= shutdown
system_call_2(8,(uint64_t)0x6cdd, 1); //= reboot;
*/
void read_flash(u64 device, u32 _flags, char *iso_path)
{
	u32 buf_size=MB(2);
...
void write_to_device()
{
    ...
    {
        if((c_firmware==4.21f || c_firmware==4.70f || c_firmware==4.75f || c_firmware==4.76f || c_firmware==4.78f || c_firmware==4.81f || c_firmware==4.82f || c_firmware==4.83f || c_firmware==4.84f) && dex_mode)
        {
            char message[512];
        ...
        else if((c_firmware==3.15f || c_firmware==3.55f || c_firmware==4.21f || c_firmware==4.46f || c_firmware==4.65f || c_firmware==4.66f || c_firmware==4.70f || c_firmware==4.75f || c_firmware==4.76f || c_firmware==4.78f || c_firmware==4.81f || c_firmware==4.82f || c_firmware==4.83f || c_firmware==4.84f) && !dex_mode)
        {
            char message[512];
...
//	exit(0);
	...
	if(!exist((char*)"/dev_flash/rebug/cobra/stage2.deh") /*&& !exist((char*)"/dev_flash/rebug/cobra/stage2.dex")*/)
	{
		if(!exist((char*)"/dev_flash/rebug/cobra/stage2.deh.bak") /*&& !exist((char*)"/dev_flash/rebug/cobra/stage2.dex.bak")*/)
		{
...
//====================================================================
	...
	if(peekq(0x8000000000312AE8ULL)==DEH) {dex_mode=2; c_firmware=3.15f;}
	else
	if(peekq(0x80000000002CFF98ULL)==CEX) {dex_mode=0; c_firmware=3.15f;}
	else
	...
	if(peekq(0x800000000032EB60ULL)==DEH && peekq(0x8000000000344B70ULL)==0x323031352F31322FULL) {dex_mode=2; c_firmware=4.78f;} //timestamp: 2015/12 
	else
	if(peekq(0x800000000032EB60ULL)==DEH && peekq(0x8000000000344B70ULL)==0x323031362F31302FULL) {dex_mode=2; c_firmware=4.81f;} //timestamp: 2016/10
	else
	if(peekq(0x800000000032EB60ULL)==DEH && peekq(0x8000000000344B70ULL)==0x323031372F30392FULL) {dex_mode=2; c_firmware=4.82f;} //timestamp: 2017/09
	else
	if(peekq(0x800000000032EB60ULL)==DEH && peekq(0x8000000000344B70ULL)==0x323031382F30392FULL) {dex_mode=2; c_firmware=4.83f;} //timestamp: 2018/09
	else		
	if(peekq(0x800000000032EB60ULL)==DEH && peekq(0x8000000000344B70ULL)==0x323031392F30312FULL) {dex_mode=2; c_firmware=4.84f;} //timestamp: 2019/01
	else				
	if(c_firmware == 0)
    {
	...
	if(c_firmware==3.15f && dex_mode)
	{
		HVSC_SYSCALL_ADDR		= HVSC_SYSCALL_ADDR_315;
		NEW_POKE_SYSCALL_ADDR	= NEW_POKE_SYSCALL_ADDR_315;
		SYSCALL_TABLE			= SYSCALL_TABLE_315;
	}
...
force_reload:
		...
		(xmb[xmb_icon].member[xmb[xmb_icon].first].type==6 || xmb[xmb_icon].member[xmb[xmb_icon].first].type==7))
	{
		...
		if(xmb_icon==1) // Home/System column functions
		{
			...
			if(xmb[xmb_icon].first==3) {shutdown_system(0x10);}			// shutdown PS3
		}
		...
		if(xmb_icon==5) // Utilities
		{
			...
			if(xmb[xmb_icon].first==n+0) {create_packages();}
			if(xmb[xmb_icon].first==n+1) {export_lv(1);} //lv1
			if(xmb[xmb_icon].first==n+2) {export_lv(0);} //lv2
			if(xmb[xmb_icon].first==n+3) {dump_flash();}
            if((!dex_mode && (c_firmware==3.15f || c_firmware==3.55f || c_firmware==4.46f || c_firmware==4.65f || c_firmware==4.66f) ) || c_firmware==4.21f  || c_firmware==4.70f || c_firmware==4.75f || c_firmware==4.76f || c_firmware==4.78f || c_firmware==4.81f || c_firmware==4.82f || c_firmware==4.83f  || c_firmware==4.84f )
			{
			  if(xmb[xmb_icon].first==n+4) {dump_root_key();}
			if(xmb[xmb_icon].first==n+5) {enable_hvproc();}		
			if(xmb[xmb_icon].first==n+6) {fix_sha1();}				
			}
		}
...
{
int found=0;
        for(u64 addr=0xA000; addr<0x800000ULL; addr+=4)
		{
            if(peek_lv1_cobra(addr) == 0x7C0403787F83E378ULL){ //static search pattern for dbg level function
                poke_lv1(addr+4, 0x7F83E37860000000ULL);
		found++;
            }
            if(peek_lv1_cobra(addr) == 0x7FA3EB783BA00000ULL){ //static search pattern for dbg level function
                poke_lv1(addr+4, 0x3BA0000060000000ULL);
		found++;
            }
            if(peek_lv1_cobra(addr) == 0x419E0018F8010070ULL){ //static search pattern for dbg level function
                poke_lv1(addr, 0x60000000F8010070ULL);
		found++;
            }
            if(peek_lv1_cobra(addr) == 0x7F8B0000419D0014ULL){ //static search pattern for dbg level function
                poke_lv1(addr, 0x7F8B000060000000ULL);
		found++;
			}
	if(found==4)
	{
	...
	dialog_ret=0;
	if(found==4)
		cellMsgDialogOpen2( type_dialog_ok, (const char*) "Successfully Enabled HVPROC.", dialog_fun2, (void*)0x0000aaab, NULL );
	else
		cellMsgDialogOpen2( type_dialog_ok, (const char*) "Failed to enable HVPROC", dialog_fun2, (void*)0x0000aaab, NULL );
	wait_dialog_simple();
}
void load_lv2_kernel(u8 _sel)
{
...
void check_settings()
{
	...
	else if(c_firmware==4.65f || c_firmware==4.66f || c_firmware==4.70f || c_firmware==4.75f || c_firmware==4.76f || c_firmware==4.78f || c_firmware==4.81f || c_firmware==4.82f || c_firmware==4.83f  || c_firmware==4.84f ) // Fixed
	{
		if(  peek_lv1_cobra(0x309E4C       ) == 0xE8830018E8840000ULL)	lv1_pp=1;	else lv1_pp=0;    // Fixed   IDA
		if( (peek_lv1_cobra(0x2b4434) >> 32) == 0x60000000ULL)			lv1_lv2=1;	else lv1_lv2=0;   // Fixed   IDA
		if( (peek_lv1_cobra(0x2DD70C) >> 32) == 0x60000000ULL)			lv1_htab=1; else lv1_htab=0;  // Fixed   IDA
		...
		if( (peek_lv1_cobra(0x7984CC) >> 32) == 0x6400FFFFULL)  	lv1_smgo=1; else lv1_smgo=0;  // DEH NAND
		if( (peek_lv1_cobra(0x11B090) >> 32) == 0x38600001ULL)  	lv1_go=1; else lv1_go=0; // DEH NAND
		
	    /*for(u64 addr=0xA000; addr<0x800000ULL; addr+=4)
        if(peek_lv1_cobra(addr) == 0x7C0403787F83E378ULL) 			lv1_debug=1;	else lv1_debug=0;   	*/

	
	
	}
	else if(c_firmware==3.15f) // Fixed
	{
		if(  peek_lv1_cobra(0x3021C0       ) == 0xE8830018E8840000ULL)	lv1_pp=1;	else lv1_pp=0; // yes
		//if( (peek_lv1_cobra(0x2AE200) >> 32) == 0x60000000ULL)			lv1_lv2=1;	else lv1_lv2=0; // yes
		if( (peek_lv1_cobra(0x2D5540) >> 32) == 0x60000000ULL)			lv1_htab=1; else lv1_htab=0; // yes
		if( (peek_lv1_cobra(0x1B46B0) >> 32) == 0x38600000ULL)			lv1_indi=1; else lv1_indi=0; // Yes
		if( (peek_lv1_cobra(0x706B60) >> 32) == 0x38000000ULL)			lv1_um=1;	else lv1_um=0; //yes
		if( (peek_lv1_cobra(0x177754) >> 32) == 0x38600000ULL)			lv1_dm=1;	else lv1_dm=0; //yes

		if( (peek_lv1_cobra(0x273A64) >> 32) == 0x3920005FULL)			lv1_enc=1;	else lv1_enc=0; //yes
		if( (peek_lv1_cobra(0x70418C) >> 32) == 0x60000000ULL)			lv1_pkg=1;	else lv1_pkg=0; // yes
		if( (peek_lv1_cobra(0x2DD4EC) >> 32) == 0xE81E0020ULL)			lv1_lpar=1;	else lv1_lpar=0; //yes
		if( (peek_lv1_cobra(0x2F2010) >> 32) == 0x3920FFFFULL)			lv1_spe=1;	else lv1_spe=0; //yes
		if( (peek_lv1_cobra(0x2E353C) >> 32) == 0x3800000FULL)			lv1_dabr=1;	else lv1_dabr=0; //yes

		if( (peek_lv1_cobra(0x215030) >> 32) == 0x38001000ULL)			lv1_gart=1;	else lv1_gart=0; //yes
		if( (peek_lv1_cobra(0x71BAA0) >> 32) == 0x60000000ULL)			lv1_keys=1;	else lv1_keys=0; //yes
		if( (peek_lv1_cobra(0x25B64C) >> 32) == 0x38600001ULL)			lv1_acl=1;	else lv1_acl=0; //yes

		/* Value changes depending on kernel type*/
        lv1_smgo = lv1_go = 0;
			/* NAND */
		if(!is_nor() &&(cid==0x81) &&((peek_lv1_cobra(0x36C260) >> 32) == 0x6400FFFFULL) ) lv1_smgo=1;
		if(!is_nor() &&(cid==0x81) &&((peek_lv1_cobra(0x11EB20) >> 32) == 0x38600001ULL) ) lv1_go=1;
	}
	...
	//rebug_checked=1;
	cobra_checked=1;
	if( exist((char*)"/dev_flash/rebug/cobra/stage2.deh") /*&&
	...
	{
		if( exist((char*)"/dev_flash/rebug/cobra/stage2.deh.bak") /*&&
...
/*				strcpy(status1, "REBUG");
				...
				strcpy(select2, "    R1+CROSS TO USE: [DEX XMB]");
...
/*				strcpy(status1, "REBUG");
				...
				strcpy(status3, "DEBUG");
				strcpy(select1, "   L1+CROSS TO SET: [NORMAL MODE]");
				strcpy(select2, "   R1+CROSS TO USE: [DECR XMB]");
				strcpy(select3, "     R2+CROSS TO USE: [MENU 1]");
				mode_select = 0;
				xmb_select = 2;
				menu_select = 1;
...
void change_lv1_um(u8 val)
{
	if(c_firmware==3.15f)
	{
		u64 org= peek_lv1_cobra(0x706B60) & 0x00000000FFFFFFFFULL;
		if(val)	poke_lv1(0x706B60, 0x3800000000000000ULL | org);
		else	poke_lv1(0x706B60, 0xE818000800000000ULL | org);
	}
	...
	if(c_firmware==4.50f ||  c_firmware==4.53f ||  c_firmware==4.55f ||  c_firmware==4.60f ||  c_firmware==4.65f ||  c_firmware==4.66f || c_firmware==4.70f || c_firmware==4.75f || c_firmware==4.76f || c_firmware==4.78f || c_firmware==4.81f || c_firmware==4.82f || c_firmware==4.83f  || c_firmware==4.84f ) // Fixed
	{
		u64 org=peek_lv1_cobra(0x706BD4) & 0x00000000FFFFFFFFULL;
		if(val)	poke_lv1(0x706BD4, 0x3800000000000000ULL | org);
		else	poke_lv1(0x706BD4, 0xE818000800000000ULL | org);
	}
}
...
void change_lv1_dm(u8 val)
{
	...
	if(c_firmware==3.15f)
	{
		u64 org=peek_lv1_cobra(0x1776B0) & 0x00000000FFFFFFFFULL;
		if(val)	poke_lv1(0x1776B0, 0x6000000000000000ULL | org); //enable patch
		else	poke_lv1(0x1776B0, 0xF801009800000000ULL | org); //restore original

		org=	peek_lv1_cobra(0x1776D4) & 0x00000000FFFFFFFFULL;
		if(val)	poke_lv1(0x1776D4, 0x3860000100000000ULL | org);
		else	poke_lv1(0x1776D4, 0x4BFFF0C500000000ULL | org);

		org=	peek_lv1_cobra(0x17774C) & 0x00000000FFFFFFFFULL;
		if(val)	poke_lv1(0x17774C, 0x3BE0000100000000ULL | org);
		else	poke_lv1(0x17774C, 0x38A1007000000000ULL | org);

		org=	peek_lv1_cobra(0x177754) & 0x00000000FFFFFFFFULL;
		if(val)	poke_lv1(0x177754, 0x3860000000000000ULL | org);
		else	poke_lv1(0x177754, 0x4800618100000000ULL | org);
	}
	...
	if(c_firmware==4.30f || c_firmware==4.31f || c_firmware==4.40f || c_firmware==4.41f || c_firmware==4.46f || c_firmware==4.50f || c_firmware==4.53f || c_firmware==4.55f || c_firmware==4.60f || c_firmware==4.65f || c_firmware==4.66f || c_firmware==4.70f || c_firmware==4.75f || c_firmware==4.76f || c_firmware==4.78f || c_firmware==4.81f || c_firmware==4.82f || c_firmware==4.83f  || c_firmware==4.84f ) // Fixed
	{
		u64 org=peek_lv1_cobra(0x177A64) & 0x00000000FFFFFFFFULL;
		if(val)	poke_lv1(0x177A64, 0x6000000000000000ULL | org); //enable patch
		else	poke_lv1(0x177A64, 0xF801009800000000ULL | org); //restore original

		org=	peek_lv1_cobra(0x177A88) & 0x00000000FFFFFFFFULL;
		if(val)	poke_lv1(0x177A88, 0x3860000100000000ULL | org);
		else	poke_lv1(0x177A88, 0x4BFFF0E500000000ULL | org);

		org=	peek_lv1_cobra(0x177B00) & 0x00000000FFFFFFFFULL;
		if(val)	poke_lv1(0x177B00, 0x3BE0000100000000ULL | org);
		else	poke_lv1(0x177B00, 0x38A1007000000000ULL | org);

		org=	peek_lv1_cobra(0x177B08) & 0x00000000FFFFFFFFULL;
		if(val)	poke_lv1(0x177B08, 0x3860000000000000ULL | org);
		else	poke_lv1(0x177B08, 0x4800606500000000ULL | org);
	}
}
...
void apply_settings(char *option, int val, u8 _forced)
{
	u8 cid=get_idps(0);
	u64 org=0;
	if(c_firmware==3.15f) // Fixed
	{
		if(!strcmp(option, "lv1_pp"))// || _forced) // Fixed
		{
			if(val)
			{
				poke_lv1(0x3021C0 +  0, 0xE8830018E8840000ULL);
				poke_lv1(0x3021C0 +  8, 0xF88300C84E800020ULL);
				poke_lv1(0x3021C0 + 16, 0x38000000E8A30020ULL);
				poke_lv1(0x3021C0 + 24, 0xE8830018F8A40000ULL);
			}
			else
			{
				poke_lv1(0x3021C0 +  0, 0x6400FFFF6000FFECULL);
				poke_lv1(0x3021C0 +  8, 0xF80300C04E800020ULL);
				poke_lv1(0x3021C0 + 16, 0x380000006400FFFFULL);
				poke_lv1(0x3021C0 + 24, 0x6000FFECF80300C0ULL);
			}
		}

		if(!strcmp(option, "lv1_htab") || _forced) // Fixed
		{
			org= peek_lv1_cobra(0x2d5540) & 0x00000000FFFFFFFFULL;
			if(val)	poke_lv1(0x2d5540, 0x6000000000000000ULL | org);
			else	poke_lv1(0x2d5540, 0x41DA005400000000ULL | org);
		}

		if(!strcmp(option, "lv1_indi") || _forced) // Fixed
		{
			org= peek_lv1_cobra(0x1B46B0) & 0x00000000FFFFFFFFULL;
			if(val)	poke_lv1(0x1B46B0, 0x3860000000000000ULL | org);
			else	poke_lv1(0x1B46B0, 0x7C63003800000000ULL | org);
		}
		...
		{
			org=	peek_lv1_cobra(0x273A64) & 0x00000000FFFFFFFFULL;
			if(val)	poke_lv1(0x273A64, 0x3920005F00000000ULL | org);
			else	poke_lv1(0x273A64, 0x3920004F00000000ULL | org);
		}
		
                if((cid==0x81) &&!is_nor() && (_forced || !strcmp(option, "lv1_smgo")) ) // CEX 3.55 , NAND
		{
			org=	peek_lv1_cobra(0x36C260) & 0x00000000FFFFFFFFULL;
			if(val)	poke_lv1(0x36C260, 0x6400FFFF00000000ULL | org);
			else	poke_lv1(0x36C260, 0x6400003B00000000ULL | org);

			if(val)	poke_lv1(0x36C264, 0xF93F01C86000FFFEULL);
			else	poke_lv1(0x36C264, 0xF93F01C86000F7EEULL);
		}

		if(!strcmp(option, "lv1_pkg") || _forced) // Fixed
		{
			org=	peek_lv1_cobra(0x70418C) & 0x00000000FFFFFFFFULL;
			if(val)	poke_lv1(0x70418C, 0x6000000000000000ULL | org);
			else	poke_lv1(0x70418C, 0x419D00A800000000ULL | org);
		}
		...
		if(!strcmp(option, "lv1_lpar") || _forced)
		{
			if(val)
			{
				poke_lv1(0x2DCDC4 +  0, 0xE81E0020E95E0028ULL);
				poke_lv1(0x2DCDC4 +  8, 0xE91E0030E8FE0038ULL);
				poke_lv1(0x2DCDC4 + 12, 0xE8FE0038EBFE0018ULL);

				poke_lv1(0x2DD048 +  0, 0xE81E0020E93E0028ULL);
				poke_lv1(0x2DD048 +  8, 0xE95E0030E91E0038ULL);
				poke_lv1(0x2DD048 + 16, 0xE8FE0040E8DE0048ULL);
				poke_lv1(0x2DD048 + 20, 0xE8DE0048EBFE0018ULL);

				poke_lv1(0x2DD4EC +  0, 0xE81E0020E93E0028ULL);
				poke_lv1(0x2DD4EC +  8, 0xE95E0030E91E0038ULL);
				poke_lv1(0x2DD4EC + 16, 0xE8FE0040E8DE0048ULL);
				poke_lv1(0x2DD4EC + 20, 0xE8DE0048EBFE0018ULL);

			}
			else
			{
				poke_lv1(0x2DCDC4 +  0, 0xE81E0018E95E0020ULL);
				poke_lv1(0x2DCDC4 +  8, 0xE91E0028E8FE0030ULL);
				poke_lv1(0x2DCDC4 + 12, 0xE8FE0030EBEB0050ULL);

				poke_lv1(0x2DD048 +  0, 0xE81E0018E93E0020ULL);
				poke_lv1(0x2DD048 +  8, 0xE95E0028E91E0030ULL);
				poke_lv1(0x2DD048 + 16, 0xE8FE0038E8DE0040ULL);
				poke_lv1(0x2DD048 + 20, 0xE8DE0040EBEB0050ULL);

				poke_lv1(0x2DD4EC +  0, 0xE81E0018E93E0020ULL);
				poke_lv1(0x2DD4EC +  8, 0xE95E0028E91E0030ULL);
				poke_lv1(0x2DD4EC + 16, 0xE8FE0038E8DE0040ULL);
				poke_lv1(0x2DD4EC + 20, 0xE8DE0040EBEB0050ULL);
			}
		}

		if(!strcmp(option, "lv1_spe") || _forced) // Fixed IDA
		{
			org=	peek_lv1_cobra(0x2F2010) & 0x00000000FFFFFFFFULL;
			if(val)	poke_lv1(0x2F2010, 0x3920FFFF00000000ULL | org);
			else	poke_lv1(0x2F2010, 0x3920000900000000ULL | org);
		}

		if(!strcmp(option, "lv1_dabr") || _forced) // Fixed IDA
		{
			org=	peek_lv1_cobra(0x2E353C) & 0x00000000FFFFFFFFULL;
			if(val)	poke_lv1(0x2E353C, 0x3800000F00000000ULL | org);
			else	poke_lv1(0x2E353C, 0x3800000B00000000ULL | org);
		}

		if(!strcmp(option, "lv1_gart") || _forced) // Fixed IDA
		{
			org=	peek_lv1_cobra(0x215030) & 0x00000000FFFFFFFFULL;
			if(val)	poke_lv1(0x215030, 0x3800100000000000ULL | org);
			else	poke_lv1(0x215030, 0x3C00000100000000ULL | org);
		}

		if(!strcmp(option, "lv1_keys") || _forced) // Fixed
		{
			org=	peek_lv1_cobra(0x71baa0) & 0x00000000FFFFFFFFULL;
			if(val)	poke_lv1(0x71baa0, 0x6000000000000000ULL | org);
			else	poke_lv1(0x71baa0, 0x419D004C00000000ULL | org);
		}
		...
		if(!strcmp(option, "lv1_acl") || _forced) // Fixed IDA
		{
			if(val)
			{
				poke_lv1(0x25b64C + 0, 0x386000012F830000ULL);
				poke_lv1(0x25b64C + 8, 0x419E001438000001ULL);
			}
			else
			{
				poke_lv1(0x25b64C + 0, 0x5463063E2F830000ULL);
				poke_lv1(0x25b64C + 8, 0x419E0014E8010070ULL);
			}
		}
		if((cid==0x81) &&!is_nor() && (_forced || !strcmp(option, "lv1_go")) ) // DEH 3.15 NAND
		{
			org=	peek_lv1_cobra(0x11EB20) & 0x00000000FFFFFFFFULL;
			if(val)	poke_lv1(0x11EB20, 0x3860000100000000ULL | org);
			else	poke_lv1(0x11EB20, 0x3860000000000000ULL | org);
		}
	...
	if(c_firmware==4.65f || c_firmware==4.66f || c_firmware==4.70f || c_firmware==4.75f || c_firmware==4.76f || c_firmware==4.78f || c_firmware==4.81f || c_firmware==4.82f || c_firmware==4.83f  || c_firmware==4.84f ) // Fixed
	{
		if(!strcmp(option, "lv1_pp"))// || _forced) // Fixed
		{
			if(val)
		...
		{
			org = peek_lv1_cobra(0x7984CC) & 0x00000000FFFFFFFFULL;
			if(val) poke_lv1(0x7984CC, 0x6400FFFF00000000ULL | org);
			else    poke_lv1(0x7984CC, 0x640003FB00000000ULL | org);

			if(val) poke_lv1(0x7984D0, 0xF93F01D06000FFFEULL);
			else    poke_lv1(0x7984D0, 0xF93F01D06000F7EEULL);
		}
		...
		if(!strcmp(option, "lv1_debug") || _forced) //CEX 4.65-4.75 , NOR
		{	
        for(u64 addr=0xA000; addr<0x800000ULL; addr+=4)
		{ //search lv1
            if(peek_lv1_cobra(addr) == 0x7C0403787F83E378ULL){ //static search pattern for dbg level function
...
#define LANG_DIR             "/dev_hdd0/tmp/wm_lang"
	...
	{
		// [DECR XMB]: REBUG MODE ONLY
		if(xmb_mode==0) //switch to DECR XMB //xmb_select == 2)
		{
		  result = stat("/dev_rebug/vsh/module/vsh.self.nrm", &statinfo);
		  if(result == 0 )
		  {
			result = stat("/dev_rebug/vsh/module/vsh.self.dehsp", &statinfo);
			if(result == 0 )
			{
				if(rename ("/dev_rebug/vsh/module/vsh.self","/dev_rebug/vsh/module/vsh.self.dexsp" )== 0 &&
					rename ("/dev_rebug/vsh/module/vsh.self.dehsp","/dev_rebug/vsh/module/vsh.self" )== 0)
				{
					...
					strcpy(status, "[DECR XMB] is now active.\n\nSystem will auto-reboot when you quit.");
					auto_reboot = 1;
				}
				else
				{
			...
			{
				strcpy(status, "PLEASE TRY [DEX XMB]");
			}
		  }
		  else
		  {
		...
		// [DEX XMB]: REBUG MODE ONLY
		if(xmb_mode==1) // switch to DEX XMB //xmb_select == 1)
		{
		  result = stat("/dev_rebug/vsh/module/vsh.self.nrm", &statinfo);
		  if(result == 0 )
		  {
			...
			if(result == 0 )
			{
				if(rename ("/dev_rebug/vsh/module/vsh.self","/dev_rebug/vsh/module/vsh.self.dehsp" )== 0 &&
					rename ("/dev_rebug/vsh/module/vsh.self.dexsp","/dev_rebug/vsh/module/vsh.self" )== 0)
				{
					strcpy(status, "[DEX XMB] is now active.\n\nSystem will auto-reboot when you quit.");
					auto_reboot = 1;
				}
				else
				{
			...
			{
				strcpy(status, "PLEASE TRY [DECR XMB]");
			}
		  }
		  else
		  {
	...
	if(!strcmp(option, "cobra_mode"))
	{
		...
		{
			result = stat("/dev_rebug/rebug/cobra/stage2.deh.bak", &statinfo);
			if(result == 0)
			{
				rename ("/dev_rebug/rebug/cobra/stage2.deh.bak","/dev_rebug/rebug/cobra/stage2.deh" );
		...
		{
			result = stat("/dev_rebug/rebug/cobra/stage2.deh", &statinfo);
			if(result == 0)
			{
				rename ("/dev_rebug/rebug/cobra/stage2.deh","/dev_rebug/rebug/cobra/stage2.deh.bak" );
...
void fix_sha1()
{
    ...
    int bsize=0;
    uint8_t buffer_infile[131072];
    unsigned char buffer_sha1[40];
    FILE *fd=NULL;

    u8 crl[0x40]={0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x2C, 0x6E, 0x47, 0x15, 0xE8, 0x38, 0x9B, 0xC8, 0x16, 0x65, 0x6E, 0x0C, 0x37, 0x54, 0x25, 0xFE, 0x7B, 0x22, 0x9A, 0x31, 0x75, 0x72, 0x22, 0x63, 0x2B, 0x31, 0xDD, 0x15, 0xAA, 0x60, 0x7D, 0xEB, 0xF5, 0xF7, 0xA3, 0x74, 0x0B, 0x9D, 0xDD, 0x3B, 0x3A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    u8 drl[0x40]={0x10, 0x00, 0x00, 0x0C, 0x00, 0x04, 0x10, 0x03, 0x00, 0x00, 0x00, 0x03, 0x20, 0x00, 0x00, 0x34, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x94, 0x98, 0xDD, 0x6F, 0xC5, 0x64, 0xC9, 0x14, 0xB1, 0x37, 0x03, 0xF3, 0x7C, 0x17, 0x87, 0x60, 0x10, 0xDB, 0xA1, 0xFA, 0x80, 0x71, 0xBB, 0xF7, 0x9E, 0xB2, 0x6B, 0x54, 0xC5, 0xFC, 0xFE, 0x98, 0x33, 0xB7, 0x14, 0x80, 0xDC, 0xEC, 0x67, 0x5B};

    cellFsMkdir((char*)"/dev_flash3/data-revoke", CELL_FS_S_IFDIR | 0777);
    cellFsMkdir((char*)"/dev_flash3/data-revoke/drl", CELL_FS_S_IFDIR | 0777);
    cellFsMkdir((char*)"/dev_flash3/data-revoke/crl", CELL_FS_S_IFDIR | 0777);
    cellFsMkdir((char*)"/dev_flash3/data-revoke/drl", CELL_FS_S_IFDIR | 0777);
    cellFsMkdir((char*)"/dev_flash3/data-revoke/ocrl", CELL_FS_S_IFDIR | 0777);
    cellFsMkdir((char*)"/dev_flash3/data-revoke/srm", CELL_FS_S_IFDIR | 0777);

    sprintf(shafile, "/dev_flash3/data-revoke/crl/CRL1");
    if(!exist(shafile) || !exist((char*)"/dev_flash3/data-revoke/crl/CRL2"))
    {
        memset(&buffer_infile, 0, 131072);
        memcpy(&buffer_infile, &crl, 0x40);
        array_to_file(shafile, buffer_infile, 131072);

        sprintf(shafile, "/dev_flash3/data-revoke/crl/CRL2");
        memset(&buffer_infile, 0, 131072);
        memcpy(&buffer_infile, &crl, 0x40);
        array_to_file(shafile, buffer_infile, 131072);
    }

    sprintf(shafile, "/dev_flash3/data-revoke/drl/DRL1");
    if(!exist(shafile) || !exist((char*)"/dev_flash3/data-revoke/drl/DRL2"))
    {
        memset(&buffer_infile, 0, 131072);
        memcpy(&buffer_infile, &drl, 0x40);
        array_to_file(shafile, buffer_infile, 32768);

        sprintf(shafile, "/dev_flash3/data-revoke/drl/DRL2");
        memset(&buffer_infile, 0, 131072);
        memcpy(&buffer_infile, &drl, 0x40);
        array_to_file(shafile, buffer_infile, 32768);
    }
    ...
    sprintf(shafile, "/dev_flash3/data-revoke/drl/DRL1");
    bsize=is_size(shafile);

    fd = fopen(shafile, "rb");
    if(fd!=NULL)
    {
        fseek(fd, 0, SEEK_SET);
        fread((char*) &buffer_infile, bsize, 1, fd);
        fclose(fd);

        sha1(buffer_infile, bsize, buffer_sha1);
    }
    else
    {
        cellMsgDialogAbort();
        dialog_ret=0;
        cellMsgDialogOpen2( type_dialog_ok, (const char*) "This operation is not allowed.", dialog_fun2, (void*)0x0000aaab, NULL );
        wait_dialog_simple();
        return;
    }
    ...
    sprintf(shafile, "/dev_flash3/data-revoke/crl/CRL1");
    bsize=is_size(shafile);
	if(bsize>131072) { /*** show message ***/ ; return;} // AV
    fd = fopen(shafile, "rb");
    if(fd!=NULL)
    {
        fseek(fd, 0, SEEK_SET);
        fread((char*) &buffer_infile, bsize, 1, fd);
        fclose(fd);
        sha1(buffer_infile, bsize, buffer_sha1+20);
    }
    else
    {
        cellMsgDialogAbort();
    ...
    u8 retries=0;
    u8*vdata=(u8*)memalign(16, 4096);
    u8 is_match=2;

    for(u8 m=0;m<10;m++)
    {
recheck:
        int ret2=vtrm_retrieve(vdata);
        if(ret2==40)
        {
            is_match=1;
            ...
            for(u8 n=0;n<40;n++)
            {
                if(vdata[n]!=buffer_sha1[n]) {is_match=0; break;}
            }
            if(!is_match)
            {
                memset(vdata, 0, 4096);
                memcpy(vdata, &buffer_sha1, 40);
                for(u8 k=0;k<10;k++)
                {
                    int ret3=vtrm_update(vdata);
                    if(ret3>=1)
                    {
                        is_match=2;
                        retries++;
                ...
                cellMsgDialogAbort();
                dialog_ret=0;
                cellMsgDialogOpen2( type_dialog_ok, (const char*) "Timeout has occurred while performing the operation, please try again.", dialog_fun2, (void*)0x0000aaab, NULL );
                wait_dialog_simple();
                return;
            }
            break;
    ...
    if(retries && retries<6)
    {
        dialog_ret=0;
        cellMsgDialogOpen2( type_dialog_ok, (const char*) "Blu-ray\xE2\x84\xA2 movie playback is restored. Please press OK to restart your PLAYSTATION\xC2\xAE\x33 System.", dialog_fun2, (void*)0x0000aaab, NULL );
        wait_dialog_simple();
        exit(0);
    }
    else
    {
        dialog_ret=0;
        cellMsgDialogOpen2( type_dialog_ok, (const char*) "It seems there is nothing wrong with Blu-ray\xE2\x84\xA2 movie playback on your PLAYSTATION\xC2\xAE\x33 System.", dialog_fun2, (void*)0x0000aaab, NULL );
        wait_dialog_simple();
    }
}
